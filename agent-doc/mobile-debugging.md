...
3. 遵守 `AGENTS.md` 的 [Agent Temporary Files](../AGENTS.md#agent-temporary-files) 规则：只用 Bilisound Expo Dev Client（`moe.bilisound.app.dev`），不用 Expo Go；临时截图、快照、日志、pid 文件写入仓库根目录 `.temp/`。
4. 先检查 Android dev client 是否可能落后于 native 相关提交：
...
5. 用 agent 专用脚本启动 Metro 并打开 dev app：
...
`agent:android:metro` 先执行 `adb reverse tcp:8081 tcp:8081`，再用 `agent-device metro prepare` 启动或复用 Expo Metro；`agent:android:open` 以 `bilisound-tablet` 会话重启 dev app。需要手动执行或调整参数时，以 `apps/mobile/package.json` 中这两个脚本的命令为准。不要裸跑长期运行的 `expo start`。

6. 首屏通常是 Expo Dev Client 服务器列表，不是业务主界面。选择 `http://127.0.0.1:8081` 对应的 `Bilisound Dev`。
7. 看到 `歌单`、`查询`、`设置` 后，才算进入业务主界面。
8. 若 `snapshot -i` 为空，或 raw tree 只看到 `ComposeView` / `Tools`，通常是 Expo Dev Client 右上角 `Tools button` 浮层污染了 accessibility tree。打开 Expo dev menu，关闭 `Tools button` 开关，再重新抓 snapshot。
9. 当前未发现项目级 npm / Expo 配置能在启动时强制关闭 `Tools button`。不要用固定坐标脚本假装自动关闭；该开关属于设备本地 dev menu 状态，先手动关闭一次更可靠。
...
```bash
curl -s "http://127.0.0.1:8081/.expo/.virtual-metro-entry.bundle?platform=android&dev=true&minify=false" -o .temp/bundle.js
# 在仓库根目录执行；检查你最近的改动标记是否出现在 bundle 里
```
